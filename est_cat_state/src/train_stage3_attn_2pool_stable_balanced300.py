# train_stage3_attn_2pool_stable_balanced300.py
# Stage3.2 (Stable): Task-specific Attention Pooling (2pool)
# + Class imbalance handling (target ~300 per class via WeightedRandomSampler)
#
# 핵심 아이디어:
# - 너의 샘플 단위는 "프레임 한 장"이 아니라 "비디오폴더(프레임 K장 묶음)" 1개 = 1샘플임.
# - action/emotion 둘 다 불균형이 심하므로, 한쪽만 300으로 강제하면 다른쪽 분포가 깨질 수 있음.
# - 그래서 (action, emotion) 동시 균형을 노리는 샘플 가중치로 "부족 클래스는 더 자주", "많은 클래스는 덜" 뽑히게 함.
# - epoch당 샘플 수(num_samples_per_epoch)를 300 * max(n_action, n_emo)로 잡으면
#   "각 클래스가 한 epoch에 대략 300번 뽑히는" 쪽으로 유도 가능.
...
class CFG:
    ...
    target_per_class: int = 300  # "각 300장씩"에 해당하는 목표치 (샘플=비디오폴더 단위)
    sampler_strategy: str = "dual_prod"  # ["dual_prod", "pair"]
    # dual_prod: w = 1/(count(action)*count(emotion)) -> action/emotion 동시 균형 유도
    # pair:      w = 1/(count(action,emotion pair))   -> pair 균형 유도 (pair 종류 많으면 불안정/폭증 가능)
    ...
    out_root: Path = Path("./runs_stage3_2pool_stable_balanced300")
...
# =========================
def print_class_distribution(items, n_action, n_emo, title="TRAIN"):
    act_cnt = Counter([a for _, a, _ in items])
    emo_cnt = Counter([e for _, _, e in items])
    print(f"[DIST:{title}] action classes={n_action}  emotion classes={n_emo}")
    print(f"[DIST:{title}] action count (min/mean/max) = "
          f"{min(act_cnt.values())}/{np.mean(list(act_cnt.values())):.2f}/{max(act_cnt.values())}")
    print(f"[DIST:{title}] emotion count (min/mean/max) = "
          f"{min(emo_cnt.values())}/{np.mean(list(emo_cnt.values())):.2f}/{max(emo_cnt.values())}")


def make_sampler_weights(items, strategy="dual_prod"):
    """
    items: List[(vdir, action_id, emo_id)]
    strategy:
      - "dual_prod": w = 1 / (count(action) * count(emotion))
      - "pair":      w = 1 / count((action,emotion))
    """
    act_cnt = Counter([a for _, a, _ in items])
    emo_cnt = Counter([e for _, _, e in items])

    if strategy == "pair":
        pair_cnt = Counter([(a, e) for _, a, e in items])
        ws = []
        for _, a, e in items:
            ws.append(1.0 / max(1, pair_cnt[(a, e)]))
        return torch.DoubleTensor(ws)

    # default: dual_prod
    ws = []
    for _, a, e in items:
        ws.append(1.0 / (max(1, act_cnt[a]) * max(1, emo_cnt[e])))
    return torch.DoubleTensor(ws)
...
def train(cfg: CFG):
    ...
    run_name = (
        f"{ts}_bb-{cfg.backbone}_stage3_2poolSTABLE_BAL300_K{cfg.frames_per_video}"
        ...
        f"_sampler{int(cfg.use_weighted_sampler)}-{cfg.sampler_strategy}_T{cfg.target_per_class}"
    )
    ...
    print_class_distribution(tr_items, len(action2id), len(emo2id), title="TRAIN")

    encoder, feat_dim, model_img = create_backbone(cfg.backbone)

    # augmentation: 부족 클래스는 sampler로 더 자주 들어오므로 증강 효과가 커짐
    train_tfm = transforms.Compose([
        transforms.Resize((model_img, model_img)),
        transforms.RandomResizedCrop(model_img, scale=(0.7, 1.0)),
    ...
    # ---- Weighted sampler: "각 클래스 300개 정도" 보이게 epoch 샘플 수 설정 ----
    sampler = None
    num_samples_per_epoch = None
    if cfg.use_weighted_sampler:
        weights = make_sampler_weights(tr_items, strategy=cfg.sampler_strategy)

        # 목표: 한 epoch에서 각 클래스가 대략 target_per_class번씩 뽑히도록 유도
        # action/emotion 중 클래스 수가 더 많은 쪽을 기준으로 epoch 샘플 수를 잡음
        num_samples_per_epoch = int(cfg.target_per_class * max(len(action2id), len(emo2id)))
        sampler = WeightedRandomSampler(weights, num_samples=num_samples_per_epoch, replacement=True)
        print(f"[SAMPLER] enabled strategy={cfg.sampler_strategy} num_samples/epoch={num_samples_per_epoch} (replacement=True)")

        tr_loader = DataLoader(
            tr_ds, batch_size=cfg.batch_size, sampler=sampler, shuffle=False,
    ...
    if cfg.init_ckpt:
        try:
            ...
            print(f"[WARN] init_ckpt load failed: {ex}")
    ...
    for epoch in range(1, cfg.epochs + 1):
        model.train()
        pbar = tqdm(tr_loader, desc=f"[2pool-STABLE+BAL300 {cfg.backbone}] epoch {epoch}/{cfg.epochs}")
        ...
        line = (
            f"epoch={epoch} 2pool-STABLE+BAL300 K={cfg.frames_per_video} "
            ...
            torch.save(
                {
                    ...
                    "model_img": model_img,
                    "sampler": {
                        "enabled": cfg.use_weighted_sampler,
                        "strategy": cfg.sampler_strategy,
