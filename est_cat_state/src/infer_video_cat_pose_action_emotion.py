...
def run(args):
    ...
    # output fps fixed (avoid broken webm fps metadata)
    ...
    # 1초에 cls_hz번 -> 초당 N번 = frame마다 판단하려면 "프레임간 간격"을 계산
    # in_fps가 깨졌을 수 있으니, out_fps를 기준으로 추정(우리가 출력 fps를 고정하니까)
    ...
    while True:
        ...
        if picked is not None:
            ...
            sm = kp_smoother.update(None, None)  # hold previous
            # if you want: still draw last pose when holding -> we need last box/conf.
            # simplest: no draw when no detection. (if you want hold-draw, tell me)

        # --- classifier input ---
        x = preprocess_bgr(raw_frame, img_size)
        buf.append(x)

        # classify 5 times per second (cls_hz)
...
if __name__ == "__main__":
    ...
    # action/emotion inference frequency
    ...
    # keypoint smoothing
    ap.add_argument("--kp_smooth_alpha", type=float, default=0.35, help="EMA alpha (higher=more smooth)")
    ap.add_argument("--kp_min_move", type=float, default=1.5, help="deadzone pixels (ignore tiny jitter)")
    ap.add_argument("--kp_hold", type=int, default=6, help="hold last keypoints for N frames when missing")
