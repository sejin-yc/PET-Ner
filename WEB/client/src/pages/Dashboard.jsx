...
import { Battery, Zap, Mic, Volume2, Play, BrainCircuit, Repeat, Hand, Utensils, Fish, Sidebar } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      console.log("✅ [Dashboard] 제어 소켓 연결 성공");
      ws.send("CONNECT\naccept-version:1.1,1.0\nheart-beat:10000,10000\n\n\0");
    };

    ws.onclose = () => {
      console.log("❌ [Dashboard] 제어 소켓 종료");
    };
  ...
  const sendControlCommand = (key) => {
    ...
    const payload = JSON.stringify({ command: key });
    const frame = `SEND\ndestination:/pub/robot/control\ncontent-type:application/json\n\n${payload}\0`;

    ws.send(frame);
    console.log(`📤 명령 전송: ${key}`);
  };
      ...
      <div className="col-span-12 lg:col-span-8 space-y-6">
        ...
        <section className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm h-[500px] flex flex-col">
          <div className="px-4 py-3 border-b border-gray-100 font-semibold text-gray-800 flex justify-between">
            <span>My House</span>
            <span className="text-xs text-gray-400 font-normal mt-1">Map created by PET-Ner</span>
          </div>
      ...
      <div className="col-span-12 lg:col-span-4 space-y-6">
        ...
        <section className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-semibold text-gray-800 flex items-center gap-2"><Zap size={18} className="text-yellow-500" /> PET-Ner 모드</h3>
          ...
          <div className="space-y-6">
             <div className="bg-gray-50 p-1 rounded-lg flex items-center relative">
                ...
                <button onClick={() => isAuto && toggleMode()} className={`flex-1 relative z-10 py-2 text-sm font-medium transition-colors flex items-center justify-center gap-2 ${!isAuto ? 'text-indigo-600' : 'text-gray-500'}`}><Hand size={16}/> 수동 모드</button>
             </div>
             <div>
        ...
        <section className='bg-white rounded-lg border border-gray-200 shadow-sm p-4'>
          <h3 className='font-semibold text-gray-800 flex items-center gap-2 mb-3'>
            <Utensils size={18} className='text-orange-500' /> 식당
          </h3>
          <div className='grid grid-cols-2 gap-3'>
            {/* 츄르 주기 */}
            <button
              onClick={() => sendControlCommand('k')}
        ...
        <section className="bg-white rounded-lg border border-gray-200 shadow-sm p-5 space-y-5">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className='bg-gray-900 text-white px-3 rounded-lg hover:bg-black disabled:opacity-50 flex items-center justify-center min-w-[44px]'
            >
              {ttsLoading ? <div className='w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin'></div> : <Play size={16} fill='white' />}
            </button>
          </div>
          ...
          <button onMouseDown={startWalkieTalkie} onMouseUp={stopWalkieTalkie} onMouseLeave={stopWalkieTalkie} className={`w-full border-2 rounded-xl py-4 flex flex-col items-center justify-center gap-2 transition-all select-none ${isRecording ? 'border-red-500 bg-red-50 text-red-600 animate-pulse' : 'border-dashed border-gray-300 text-gray-500 hover:border-indigo-500 hover:text-indigo-600 hover:bg-indigo-50'}`}>
            ...
            <span className="text-xs font-medium">{isRecording ? "녹음 중... (떼면 전송)" : "누르고 말하기"}</span>
          </button>
        </section>
      </div>
    </div>
