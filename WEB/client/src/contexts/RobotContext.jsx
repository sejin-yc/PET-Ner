...
export const RobotProvider = ({ children }) => {
  ...
  const [isVoiceCloned, setIsVoiceCloned] = useState(() => {
    localStorage.getItem('isVoiceCloned') === 'true';
  });
    ...
    const fetchInitialState = async () => {
      try {
        ...
        if (res.data) {
          ...
          setRobotStatus(prev => ({ ...prev, mode: res.data.currentMode || 'manual', battery: res.data.battery || prev.battery }));
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    mqttClient.on('connect', () => {
      ...
      mqttClient.subscribe([statusTopic], (err) => {
        if (!err) console.log('📡 상태 토픽 구독 완료');
      });
    });
  ...
  useEffect(() => {
    ...
    api.get(`/user/voice/${user.id}/status`)
      .then((res) => { if (res.data?.hasVoice) { setIsVoiceCloned(true); setUseClonedVoice(true); } })
      .catch(() => { setIsVoiceCloned(false); setUseClonedVoice(false); });
  }, [user?.id]);
  ...
  const sendTTS = async (text) => {
    ...
    try {
      await api.post('/robot/tts', { text, useClonedVoice: isVoiceCloned && useClonedVoice, userId: user?.id });
    } catch(e) {
  ...
  const startWalkieTalkie = async () => {
    ...
    try {
      ...
      recorder.ondataavailable = (e) => {
        if (e.data.size > 0) walkieChunksRef.current.push(e.data);
      };
      ...
      recorder.onstop = async () => {
        ...
        } catch (err) { toast.error("무전 전송 실패: " + (err.response?.data?.message || err.message)); }
  ...
  const sendVoiceToServer = async (audioBlob, promptText, recordedMimeType = 'audio/webm') => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        addNotification({ type: 'robot', title: '🎤 목소리 학습 완료', message: '이제 내 목소리로 TTS를 사용할 수 있습니다.' });
      } else {
  ...
  const addTestVideo = async () => {
    try {
      ...
      const dummyData = {
        id: dummyId, userId: user?.id || 1, rentId: 999, vehicleId: 101, fileName: `test_${dummyId}.jpg`,
        url: "https://placehold.co/600x400?text=Test+Video", thumbnailUrl: "https://placehold.co/600x400?text=Test+Video",
        duration: "00:15", behavior: "테스트 감지", catName: "테스트 냥이", isLocal: true, createAt: new Date().toISOString()
      };
  ...
  const addTestLog = async () => {
    ...
    try {
      await api.post('/logs', { userId: user.id, rentId: 999, vehicleId: 101, mode: "자동 모드", status: "completed", details: "테스트 로그" });
